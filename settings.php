<?
	...
	$type = empty($type)  || !isset($type) ? 'button' : $type;
	
	$widget_type = $type == 'widget' ? 'selected' : null;
	$button_type = $type == 'button' ? 'selected' : null;
	...
	*/
	...
	$bb = $button == 'bb' ? 'selected' : "";
	$bgreen = $button == 'bgreen' ? 'selected' : "";
	$bblue = $button == 'bblue' ? 'selected' : "";
	$bred = $button == 'bred' ? 'selected' : "";
		...
		<div class="postbox" style="padding: 10px; margin: 0px;">
			<form name="emba_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
				...
				<table cellpadding="5" cellspacing="5">				
					...
					-->
					<tr valign="top"><td align="right"><b>Type</b></td><td style="padding: 10px; background: #F5F5F5;width:100%;"><select name="type"><option value="button" <?=$button_type?>>Button</option><option value="widget" <?=$widget_type?>>Widget</option></select></td></tr>
					<tr valign="top"><td align="right"><b>Location</b></td><td style="padding: 10px; background: #F5F5F5;width:100%;"><select name="display"><option value="bottom" <?=$buttom?>>Bottom of Post</option><option value="top" <?=$top?>>Top of Post</option></select></td></tr>
					<!--					
					...
					<? } else { ?>
						<tr valign="top"><td align="right"><b>Theme</b></td><td style="padding: 10px; background: #F5F5F5;width:100%;"><select name="button"><option value="bb" <?=$bb?>>Default</option><option value="bred" <?=$bred?>>Red</option><option value="bblue" <?=$bblue?>>Blue</option><option value="bgreen" <?=$bgreen?>>Green</option></select></td></tr>
						<tr valign="top"><td align="right"><b>Preview</b></td><td style="padding: 10px; background: #F5F5F5;width:100%;"><a href="http://embedarticles.com" id="embedarticles_button" data-url="null" data-key="<?=$pub_value?>" data-button-type="<?=$button?>">Embed Articles</a><script type="text/javascript" src="http://embedarticles.com/widget/button.js"></script></td></tr>
					<? } ?>
